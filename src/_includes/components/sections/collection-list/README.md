...
- **Collection Integration**: Works seamlessly with Eleventy blog collections
...
### Eleventy Blog Lists Plugin
This component requires the `eleventy-blog-lists` plugin to:
- Calculate pagination parameters
- Populate `pagingParams` with current values
- Handle page-based routing for blog listings

### Collection Setup
Requires a properly configured blog collection in Eleventy:
```javascript
// In eleventy.js
.use(collections({
  blog: {
